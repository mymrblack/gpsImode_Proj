...
void TDC_resetFifoFullFlag(void){
	MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG0_OFFSET , (0x01<<4) | 0x0c);//{m_csn, m_wrn, m_rdn, m_addr}<=slv_reg0;
	MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG0_OFFSET , (0x00<<4) | 0x0f);//{m_csn, m_wrn, m_rdn, m_addr}<=slv_reg0;
}

//tested.
void TDC_init(void){
	MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG0_OFFSET , (0x00<<4)| 0xf);
	MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG7_OFFSET , 0x1F);//Disable inputs
	return;
}
...
void TDC_RegConfig(int regNum, int configData){
	 ...
	 MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG0_OFFSET , (2<<4) | regNum);//{m_wr, m_rd, m_addr}<=slv_reg0;
	 MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG0_OFFSET , (0<<4)| 0xf);
	 return;
}
...
int TDC_ReadRegData(int regNum){
	...
	MYIMODE_mWriteReg(TDC_BASEADDR,  MYIMODE_S00_AXI_SLV_REG0_OFFSET , (1<<4) | regNum);//{m_wr, m_rd, m_addr}<=slv_reg0;
...
unsigned int TDC_GetTriTimes(int firstFlag){
	...
	startTriCoNow = MYIMODE_mReadReg(TDC_BASEADDR, REG_START_TRI_COUNTTER);
   	if(firstFlag)
...
void TDC_CheckIfNeedMasterRest(void){
	...
	TDC_RegConfig(TDC_REG_4, 0x6400000);
	TDC_ReadRegData(TDC_REG_9);
	TDC_ReadRegData(TDC_REG_8);
	return;
...
void TDC_CheckIfNeedPartialRest(void){
	...
	if( ++count == 50)
    {
		 TDC_RegConfig(TDC_REG_4, 0x6800000);
     	 count=0;
    }
	return;
}
